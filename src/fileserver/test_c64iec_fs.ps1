...
function Recv-Line([System.IO.Ports.SerialPort]$p) {
...
function Recv-RawBytes([System.IO.Ports.SerialPort]$p, [int]$count) {
    ...
    if ($read -lt $count) {
        Write-Host "  [WARN] Recv-RawBytes: expected $count bytes, got $read" -ForegroundColor Yellow
    }
    if ($read -eq 0) { return ,@() }
    return ,$buf[0..($read - 1)]
}
...
function Recv-Chunk([System.IO.Ports.SerialPort]$p) {
    $header = Recv-Line $p
...
function Recv-AllLines([System.IO.Ports.SerialPort]$p, [int]$maxLines = 100) {
    $lines = @()
